...
`worktree`::
	Analogous to `gitdir`, but the pattern provided is matched
	against the top-level directory of the working tree.

`worktree/i`::
	Same as `worktree`, except that matching is done case-insensitively.
